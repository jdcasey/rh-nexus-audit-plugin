...
import static com.redhat.tools.nexus.capture.model.CaptureSession.key;
...
import com.redhat.tools.nexus.capture.model.CaptureTarget;
import com.redhat.tools.nexus.capture.store.CaptureSessionQuery.QueryMode;
    ...
    implements CaptureStore, Initializable
{
        ...
        throws CaptureStoreException
    {
        ...
        {
            catalogs: for ( final Map.Entry<String, CaptureSessionCatalog> entry : new HashMap<String, CaptureSessionCatalog>(
                                                                                                                               catalogs ).entrySet() )
            {
        ...
        throws CaptureStoreException
    {
        ...
        {
            throw new CaptureStoreException(
                                             "Cannot log capture-session to disk. Failed to create storage directory: {0}.",
        ...
        catch ( final IOException e )
        {
            throw new CaptureStoreException( "Failed to write capture-session to disk." + "\nUser: {0}"
                + "\nBuild-Tag: {1}\nCapture-Source: {2}\nFile Path: {3}\nReason: {4}", e, session.getUser(),
                                             session.getBuildTag(), session.getCaptureSource(),
                                             sessionFile.getAbsolutePath(), e.getMessage() );
        }
        finally
        {
            IOUtil.close( writer );
        ...
        throws CaptureStoreException
    {
        ...
        {
            throw new CaptureStoreException(
                                             "Failed to write capture-session catalog to disk.\nFile: {0}\nReason: {1}",
                                             e, catalogFile.getAbsolutePath(), e.getMessage() );
        }
        finally
        {
            IOUtil.close( writer );
