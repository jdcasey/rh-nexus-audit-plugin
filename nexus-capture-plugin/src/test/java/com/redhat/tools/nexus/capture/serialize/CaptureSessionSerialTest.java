...
public class CaptureSessionSerialTest
{
        ...
        throws IllegalArtifactCoordinateException
    {
        ...
        session.add( new CaptureTarget( Arrays.asList( new String[] { "central", "codehaus", "jboss-releases" } ),
                                        "/org/groupId/artifactId/VER/artifactId-VER.pom", gavs[0], items[i++] ) );

        session.add( new CaptureTarget( Arrays.asList( new String[] { "central", "codehaus", "jboss-releases" } ),
                                        "/org/groupId/artifactId/VER/artifactId-VER.jar", gavs[0], items[i++] ) );

        session.add( new CaptureTarget( Arrays.asList( new String[] { "central", "codehaus", "jboss-releases" } ),
                                        "/org/groupId/beetifactId/VER/beetifactId-VER.pom", gavs[1], items[i++] ) );

        session.add( new CaptureTarget( Arrays.asList( new String[] { "central", "codehaus", "jboss-releases" } ),
                                        "/org/groupId/beetifactId/VER/beetifactId-VER.jar", gavs[1], items[i++] ) );

        final String result = getGson().toJson( session );
        System.out.println( result );

        getGson().fromJson( result, CaptureSession.class );
    }
